...
import ModalLanguageCreate from './ModalLanguageCreate';
...
const LanguageCatalog = () => {
    ...
    const onSubmit = data => {
        console.log(data);
        dispatch( Updating(`language/${data.id}`, data  ) );
        reset();
    }
    const headCells = [
        { id: 'id', numeric: false, disablePadding: true, label: 'ID' },
        { id: 'Languagename', numeric: false, disablePadding: false, label: 'Language' },
    ];
                    ...
                    ?   <TableCell align="left">  
                            <form onSubmit={ handleSubmit( onSubmit ) }>
                                ...
                                <TextField 
                                    inputRef={register}
                                    name="Languagename"
                                    defaultValue={row.Languagename} size="small" /> 
                                    <FormHelperText className={ classes.error }>
                                        <ErrorMessage errors={errors} name="Languagename" />
                                    </FormHelperText>
                            </form> 
                        </TableCell>   
                    :   <TableCell align="left">{row.Languagename}</TableCell>
            }
            {
                (whereUpdating === row.id)
    ...
    const handleDelete = ( id ) => ( event ) => {
        event.preventDefault();
        dispatch( Deleting(`language/${id}`, id) );
    }
