...
import ModalTypeRequesCreate from './ModalTypeRequesCreate';
...
const TypeRequestCatalog = () => {
    ...
    const schema = yup.object().shape({
        Typerequestname: yup.string().trim()
    ...
    useEffect(() => {  
        dispatch( ChargeData("typerequests/") );
    }, [dispatch]);
    ...
    useEffect(() => {
        if(search !== ''){
            setData(
                state.filter((data) =>
                    data.Typerequestname.toLowerCase().includes(search.toLowerCase())
                )
            );
        } else {
            setData( state );
    ...
    const onSubmit = data => {
        console.log(data);
        dispatch( Updating(`typerequest/${data.id}`, data  ) );
        reset();
    }
    const headCells = [
        { id: 'id', numeric: false, disablePadding: true, label: 'ID' },
        { id: 'Typerequestname', numeric: false, disablePadding: false, label: 'Type Request' },
    ];
                ...
                (whereUpdating === row.id)
                    ?   <TableCell align="left">  
                            <form onSubmit={ handleSubmit( onSubmit ) }>
                                ...
                                <TextField 
                                    inputRef={register}
                                    name="Typerequestname"
                                    defaultValue={row.Typerequestname} size="small" /> 
                                    <FormHelperText className={ classes.error }>
                                        <ErrorMessage errors={errors} name="Typerequestname" />
                                    </FormHelperText>
                            </form> 
                        </TableCell>   
                    :   <TableCell align="left">{row.Typerequestname}</TableCell>
            }
            {
                (whereUpdating === row.id)
    ...
    const handleDelete = ( id ) => ( event ) => {
        event.preventDefault();
        dispatch( Deleting(`typerequest/${id}`, id) );
    }
    ...
    return (
        <>
            ...
            <ModalCustom 
                ...
                component={ ModalTypeRequesCreate }
                />
        </>
    )
}

export default TypeRequestCatalog
